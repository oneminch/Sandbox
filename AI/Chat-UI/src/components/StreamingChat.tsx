...
const Chat: React.FC = () => {
	...
	const { messages, isStreaming, sendMessage, currentStreamedContent } =
		useStreamingChat();
	...
	return (
		<div className="max-w-5xl mx-auto flex min-h-screen flex-col gap-6">
			<ul className="flex flex-col gap-y-4 border-x-2 border-dashed border-gray-900 px-2 pt-16 pb-28">
				...
				{isStreaming && (
					<li className="inline-block self-start max-w-[75%] bg-gray-800/75 rounded-2xl rounded-bl-none border border-gray-700 py-4 px-8">
						{currentStreamedContent}
						<span className="inline-block w-3 h-3 ml-2 bg-gray-300 rounded-full animate-pulse !duration-100"></span>
					</li>
				)}
			</ul>
			<form
